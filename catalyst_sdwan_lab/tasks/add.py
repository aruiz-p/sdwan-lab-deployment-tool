...
def main(
    ...
    if lab:
        ...
        vpn0_switch = find_node_by_label(lab, ["SW1", "VPN0-172.16.0.0/24"])
        inet_switch = find_node_by_label(lab, ["SW-INET", "INET-172.16.1.0/24"])
        mpls_switch = find_node_by_label(lab, ["SW-MPLS", "MPLS-172.16.2.0/24"])
        ...
        if cml_node_type in ["cat-sdwan-validator", "cat-sdwan-controller"]:
            ...
            if (
                ...
                for next_num_str in new_nodes_nums:
                    ...
                    variables_dict = {
                        ...
                        "csv-host-name": f"Edge{user_site_id}-{next_num_str}",
                        "//system/host-name": f"Edge{user_site_id}-{next_num_str}",
                ...
                for next_num_str in new_nodes_nums:
                    ...
                    variables_list = [
                        {
                            "name": "system_ip",
                            "value": f"10.0.{user_site_id}.{next_num_str}",
                        },
                        {"name": "host_name", "value": f"Edge{user_site_id}-{next_num_str}"},
                        {"name": "site_id", "value": int(user_site_id)},
                        ...
                        variables_list.append(
                            {
                                ...
                                "value": f"192.168.{user_site_id}.1",
                            }
                        )
                        variables_list.append(
                            {
                                "name": "vpn1_gi3_dhcp_network",
                                "value": f"192.168.{user_site_id}.0",
                            }
                        )
                        variables_list.append(
                            {
            ...
            for next_num_str in new_nodes_nums:
                ...
                next_node_x_position += 120
                node = lab.create_node(
                    label=f"Edge{user_site_id}-{next_num_str}",
                    node_definition=cml_node_type,
                    image_definition=image_definition,
